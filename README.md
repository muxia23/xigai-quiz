...
| 🔄 **错题重练** | 答错自动收录进错题本；**同一题连续答对 4 次后自动移出**；支持一键导出 PDF |
...
<details>
<summary><b>错题本是怎么收录和移除题目的？</b></summary>

任意一次答错（刷题或模拟考试中），该题会自动收录进错题本。之后**同一题每连续答对一次**计数 +1，**连续答对满 4 次即自动移出**错题本；中途只要再答错一次，连续计数清零并重新累计。也可以在「错题本」页手动点击「移出错题集」立即移除。
</details>
